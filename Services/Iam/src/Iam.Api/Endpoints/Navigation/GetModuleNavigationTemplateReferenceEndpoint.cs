...
public sealed class GetModuleNavigationTemplateReferenceEndpoint : IEndpoint
{
    ...
    {
        app.MapGet("/api/navigation/modules/reference", Handle)
            .WithName("GetModuleNavigationTemplateReference")
            .WithTags("Navigation")
    ...
    private static IResult Handle(HttpContext httpContext, IWebHostEnvironment environment)
    {
        ...
        if (!File.Exists(referencePath))
        {
            return Results.Problem(
                detail: "Localized module navigation reference JSON file is not available.",
    ...
    {
        var baseDirectory = Path.Combine(contentRootPath, "ReferenceData", "Navigation");
        var localizedPath = Path.Combine(baseDirectory, $"us_military_module_navigation_template_{language}.json");
        ...
        return Path.Combine(baseDirectory, "us_military_module_navigation_template_en.json");
    }
